...
$s1 = "INSERT INTO shepard_test SET number_correct =" . intval($_POST["number_correc"]) . ", number_task= " . intval($_POST["number_task"]) . ", average_time= " . floatval($_POST["average_time"]) . ", id_student='" . $student_id . "'" . ", array_result='" . mysql_real_escape_string($_POST["array_result"]) . "'" . ", mode= " . intval($_POST["mode"]);
$result = mysql_query($s1);

$s1 = "SELECT id FROM shepard_test WHERE id_student='" . $student_id ."'";
$result = mysql_query($s1);
$student_id = mysql_fetch_array($result, MYSQL_ASSOC);
$student_id = $student_id["id"];
echo $student_id;
